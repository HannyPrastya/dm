...
public abstract class AbstractApriori<V> {
     ...
     * @param transactions
     */
    public AbstractApriori(List<ItemSet<V>> transactions) {
     ...
     * @return
     */
    protected List<ItemSet<V>> pruneInfrequentCandidates(Double minSupport, List<ItemSet<V>> candidates) {
    ...
    public void generateRulesBase(ItemSet<V> frequentItemSet, ItemSet<V> consequent) {
        int k = frequentItemSet.size();
        int m = consequent.size();
        if (k >= m) {
            for (V s : frequentItemSet.getItems()) {
                generateRulesBase(frequentItemSet.difference(s), consequent);
            }
            ...
            rule.setConfidence(support / getAndCacheSupportForItemset(frequentItemSet));
            rules.add(rule);
        }
    }
