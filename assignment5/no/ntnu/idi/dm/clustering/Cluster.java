...
public class Cluster {
	...
	public double getDistanceToCentroid(double[] instance) {
		return EuclideanDistance.distance(centroid, instance);
	}
	...
	public int getInstanceIndexWithMaxSSE(double[][] data) {
		...
		for (int i = 0; i < indices.size(); i++) {
			currentSSE = EuclideanDistance.distance(data[indices.get(i)],
					centroid);
	...
	public double getAverageDistanceOfPointToAllPoints(double[][] data,
		...
		for (int i = 0; i < indices.size(); i++) {
			double distance = EuclideanDistance.distance(data[indices.get(i)],
					vector);
